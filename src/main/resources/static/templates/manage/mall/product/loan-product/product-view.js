layui.config({
    ...
    table.render({
        ...
        url: '/manage/mall/product/loan-label/list',
    ...
    table.on('tool(dataTableList)', function (obj) {
        ...
        if (obj.event === 'detail') {
            top.layer.open({
                ...
                maxmin:true,
                area: ['800px', '600px'],
                content: '/manage/mall/product/loan-label/view?id=' + data.id + "&rnd=" + Math.random()
            });
        } else if (obj.event === 'del') {
            layer.confirm('真的删除么', function (index) {
                $.ajax({
                    url: '/manage/mall/product/loan-label/delete',
                    data: {
                        id: data.id
                    },
            ...
            document.location.href = '/manage/mall/product/loan-label/edit?id=' + data.id + "&rnd=" + Math.random();
        }
    });
    ...
    $('#addButton').on('click', function () {
        document.location.href = '/manage/mall/product/loan-label/edit';
    });
});
