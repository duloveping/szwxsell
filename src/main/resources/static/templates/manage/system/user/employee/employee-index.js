layui.config({
    ...
    table.render({
        ...
        url: '/manage/system/user/employee/list',
    ...
    table.on('tool(dataTableList)', function (obj) {
        ...
        if (obj.event === 'detail') {
            top.layer.open({
                ...
                maxmin:true,
                area: ['800px', '600px'],
                content: '/manage/system/user/employee/view?id=' + data.id + "&rnd=" + Math.random()
            });
        } else if (obj.event === 'del') {
            layer.confirm('真的删除么', function (index) {
                $.ajax({
                    url: '/manage/system/user/employee/delete',
                    data: {
                        id: data.id
                    },
            ...
            document.location.href = '/manage/system/user/employee/edit?id=' + data.id + "&rnd=" + Math.random();
        }
    });
    ...
    $('#addButton').on('click', function () {
        document.location.href = '/manage/system/user/employee/edit';
    });
});
